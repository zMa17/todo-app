<!-- Modal Tambah Tag -->
<div id="modalTag" class="fixed inset-0 z-50 hidden items-center justify-center">
    <div class="absolute inset-0 bg-black/40" onclick="closeModalTag()"></div>
    <div class="relative bg-white border border-[#e5e5e4] rounded-lg shadow-lg w-full max-w-md mx-4 p-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-xl font-bold text-gray-900">Tambah Tag</h1>
            <button onclick="closeModalTag()" class="text-gray-400 hover:text-gray-600">
        ...
        <form action="{{ route('tag.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label for="tag_nama" class="block text-sm font-medium text-gray-700 mb-1">Nama</label>
                <input type="text" name="nama" id="tag_nama" value="{{ old('nama') }}" required
            ...
            <div>
                <label for="tag_warna" class="block text-sm font-medium text-gray-700 mb-1">Warna</label>
                <input type="color" name="warna" id="tag_warna" value="{{ old('warna', '#f59e0b') }}" required
