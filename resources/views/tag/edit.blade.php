        ...
        <div class="bg-white border p-6">
            <form action="{{ route('tag.update', $tag->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $tag->nama) }}" class="w-full px-3 py-2 border" required>
                    @error('nama') <div class="text-red-500">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="mb-1">Warna</label>
                    <input type="color" name="warna" value="{{ old('warna', $tag->warna) }}" class="border" required>
