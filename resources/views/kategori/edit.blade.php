        ...
        <div class="bg-white border p-6 rounded-lg">
            <form action="{{ route('kategori.update', $kategori->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $kategori->nama) }}" class="w-full px-3 py-2 border rounded-lg" required>
                    @error('nama') <div class="text-red-500">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="mb-1">Warna</label>
                    <input type="color" name="warna" value="{{ old('warna', $kategori->warna) }}" class="border rounded-lg" required>
                    @error('warna') <div class="text-red-500">{{ $message }}</div> @enderror
                </div>
                <button type="submit" class="px-6 py-2 bg-amber-400 text-white rounded-lg" style="border:none">Update</button>
            </form>
        </div>
    </div>
</x-app-layout>