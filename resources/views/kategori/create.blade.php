        ...
        <div class="bg-white border p-6">
            <form action="{{ route('kategori.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="color" name="warna" value="{{ old('warna', '#6366f1') }}" class="border" required>
                    @error('warna') <div class="text-red-500">{{ $message }}</div> @enderror
                </div>
                <button type="submit" class="px-6 py-2 bg-amber-400 text-white" style="border:none">Tambah</button>
            </form>
        </div>
    </div>
</x-app-layout>